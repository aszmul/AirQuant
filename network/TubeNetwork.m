...
classdef TubeNetwork < AirQuant & matlab.mixin.SetGet
    ...
    methods
        ...
        % function [g, glink, gnode] = readGraphFromTxt(filename)
        %     % READGRAPHFROMTXT Read graph structure from text file generated by Python
        %     %
        %     % [g, glink, gnode] = readGraphFromTxt(filename)
        %     %
        %     % Input:
        %     %   filename - Path to the text file containing graph data
        %     %
        %     % Output:
        %     %   g - Digraph object
        %     %   glink - Structure array containing link information
        %     %   gnode - Structure array containing node information
                
        %         % Initialize structures
        %         gnode = struct('idx', {}, 'links', {}, 'conn', {}, 'comx', {}, 'comy', {}, 'comz', {}, 'ep', {});
        %         glink = struct('n1', {}, 'n2', {}, 'point', {});
        ...
        %                 % Parse line format "node_id: [x, y, z]"
        %                 parts = split(line, ':');
        %                 node_id = str2double(parts{1});
                        
        %                 % Extract coordinates
        %                 coords_str = strtrim(parts{2});
        %                 coords = str2num(coords_str); % str2num can handle array literals
                        
        %                 % Create node entry
        %                 gnode(node_idx).idx = node_id + 1; % Convert to 1-based indexing
        %                 gnode(node_idx).links = [];
        %                 gnode(node_idx).conn = [];
        %                 gnode(node_idx).comx = coords(1);
        %                 gnode(node_idx).comy = coords(2);
        %                 gnode(node_idx).comz = coords(3);
        %                 gnode(node_idx).ep = 0;  % Will be updated later
                        
        %                 node_idx = node_idx + 1;
                        
        %             % Process edges
        %             elseif strcmp(current_section, 'edges')
        %                 % Parse line format "n1 -> n2: points"
        %                 parts = split(line, ':');
        %                 edge_parts = split(parts{1}, '->');
                        
        %                 n1 = str2double(strtrim(edge_parts{1}));
        %                 n2 = str2double(strtrim(edge_parts{2}));
                        
        %                 % Extract points
        %                 points_str = strtrim(parts{2});
        %                 points = str2num(points_str); % str2num can handle array literals
                        
        %                 % Create link entry
        %                 glink(link_idx).n1 = n1 + 1; % Convert to 1-based indexing
        %                 glink(link_idx).n2 = n2 + 1; % Convert to 1-based indexing
        %                 glink(link_idx).point = points;
                        
        %                 % Update node connectivity
        %                 node_n1_idx = find([gnode.idx] == n1 + 1);
        %                 node_n2_idx = find([gnode.idx] == n2 + 1);
                        
        %                 gnode(node_n1_idx).links = [gnode(node_n1_idx).links, link_idx];
        %                 gnode(node_n1_idx).conn = [gnode(node_n1_idx).conn, n2 + 1];
        %                 gnode(node_n2_idx).links = [gnode(node_n2_idx).links, link_idx];
        %                 gnode(node_n2_idx).conn = [gnode(node_n2_idx).conn, n1 + 1];
        ...
        %         % Update endpoint status for nodes with only one connection
        %         for i = 1:length(gnode)
        %             if length(gnode(i).conn) == 1
        %                 gnode(i).ep = 1;
        %             end
        %         end
        ...
        %             n1 = glink(i).n1;
        %             n2 = glink(i).n2;
        %             len = length(glink(i).point);
        %             A(n1, n2) = len;
        %             A(n2, n1) = len;  % Make it symmetric
        %         end
                
        %         % Find topmost node for root
        %         [~, top_idx] = max([gnode.comz]);
                
        %         % Create directed graph with direction from top to bottom
        %         g = digraph(A);
        %         % Set root node as the topmost node and create directed tree
        %         g = g.subgraph(bfsearch(g, top_idx));
        %     end
        ...
        function [g, glink, gnode] = Skel2Digraph(obj, method)
            ...
            % filename = '/home/admin-pc/work/projects/VesselQuant/uCT_Stijn_AirQuant/159/159_skel_TreeSkel_p03_LoopFree_LoopFree_graph.txt';
            % [g, glink, gnode] = readGraphFromTxt(filename);
        end
